...
func DipoleField(m, R Vector) Vector {
	...
	Bx := (mu0 / (4 * math.Pi)) * ((i3_r5 * m_R * R[X]) - (i_r3 * m[X]))
	By := (mu0 / (4 * math.Pi)) * ((i3_r5 * m_R * R[Y]) - (i_r3 * m[Y]))
	Bz := (mu0 / (4 * math.Pi)) * ((i3_r5 * m_R * R[Z]) - (i_r3 * m[Z]))
...
func doff(c, i int, m, R Vector) float64 {
	...
	m_y := m[i]
	r2 := R.Dot(R)
	pre := mu0 / (4 * math.Pi) // minus sign gives best accuracy but should not be here, what's wrong?
	return pre * (-15*x*y*m.Dot(R)/math.Pow(r2, 7./2.) + 3*y*m_x/math.Pow(r2, 5./2.) + 3*m_y*x/math.Pow(r2, 5./2.))
}
